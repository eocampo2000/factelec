...
20160509 Add method uncompressFile
20160520 Add delFilebyAge
'''
__version__ = '20160520'
...
def getFileBaseNameNoExt(fn):
    x = os.path.splitext(fn)[0]
...
# This method to trim spaces until end of file.
def trimRBlankFile(src,tgt=None):
    ...
    try:
        if tgt is None  :        
            ...
            ftmp = '%s/remBlankLineFile_%s.tmp' % (os.path.dirname(tgt), getPIDFileName(os.path.basename(tgt))) 
            fn = tgt  
...
# days file age to be deleted. Not inclusive
# 
def delFilebyAge(path, days,suf, log):
    
    if path is None  or os.path.isdir(path) == False:
        log.error('dir %s does not exist ' % path)
        return 1
    
    if days < 1 or suf is None or suf == '' :
        log.error('days cannot be less than 1. Currently days set to = %s or file suff cannot be  %s' % (days,suf))
        return 2
    
    files=[]
    now = time.time()
    cutoff = now - (days * 86400)
    #files = os.listdir(path)       # dangerous.
    for name in glob.glob('%s/*.%s' % (path,suf)):
        files.append(name)
    
    log.info ('Total files on %s files len = %s cutoff = %d ' % (path,len(files), cutoff) )
    i=0
    for fn in files:
        fnp = os.path.join(path, fn)
        age =  os.stat(fnp).st_mtime 
        if age <= cutoff:
            if os.path.isfile(fnp):
                os.remove(fnp)
                i+=1
                log.debug('Removing fnp %s' %  fnp)
    
    log.info ('Total files removed  %s ' % i)
...
#fzp archive name. Complete path 
#td dir path fpr the unzipped file.    
def uncompressFile(fzp, td,  log):
    
    if  zipfile.is_zipfile(fzp) == False:
        log.error('File %s : is not a valid ZipFile ' % fzp )
        return 1
             
    try:
        log.debug ('ZipFile (fzp) = %s  TgtDir = %s' % (fzp, td))
        zf = zipfile.ZipFile(fzp,'r')
        zf.extractall(td)
        zf.close()
        rc = 0 
