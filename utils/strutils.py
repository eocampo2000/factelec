...
20160404 Added isWeekend function.
20160509 Added getLastMonthStr function and convTimeStr.
'''
__version__ = '20160404'
import re
import time
import base64
...
def isWeekendDay(dt,fmt='%m/%d/%Y'):
    dw = getDayofWeek(dt,fmt)
    if ( dw is 'Sat' or dw is 'Sun' ) : return True
    return False
...
#This function converts One time format to another. 
# frStr string with original format    e.g 201605
# frFmt string  w source format mask   e.g.'%Y%m'
# toFmt string with target format mask e.g.'%b%y'
def convTimeStr(frStr,frFmt,toFmt):
    dtobj = datetime.strptime(frStr,frFmt)
...
if __name__ == '__main__':
    RUN_DATE = getTodayDtStr(fmt='%Y%m%d')
    RUN_DATE = '20160410'
    rc = isWeekendDay(RUN_DATE,'%Y%m%d')
    print 'RUN_DATE is ' , RUN_DATE, " rc = " , rc
